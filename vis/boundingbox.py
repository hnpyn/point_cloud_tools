...
class BoundingBox3D:
    ...
    @staticmethod
    def create_bboxes(boxes, lut=None):
        """Creates a OrientedBoundingBox that can be used to render the boxes.
        ...
        bbox3d_list = []
        for i, box in enumerate(boxes):
            rot_mat = o3d.geometry.get_rotation_matrix_from_xyz(np.asarray([0,0,-box.ori]))
            # bbox3d = o3d.geometry.OrientedBoundingBox(box.center, rot_mat, np.asarray(box.size)[[0,2,1]])
            if lut is not None and box.label_class in lut.labels:
                label = lut.labels[box.label_class]
                c = (label.color[0], label.color[1], label.color[2])
            else:
            ...
            x,y,z = box.center
            w,h,l = box.size
            bbox_mesh = o3d.geometry.TriangleMesh.create_box(w,l,h)
            t = np.asarray([x-w/2,y-l/2,z-h/2])
            bbox_mesh.translate(t)
            #bbox_mesh.translate(np.asarray([l/2,-w/2,0]))
            #box_rot_mat = o3d.geometry.get_rotation_matrix_from_xyz(np.asarray([0,0,-box.ori]))
            bbox_mesh.rotate(rot_mat, box.center.reshape([3, 1]))
            bbox_mesh.compute_triangle_normals()
            bbox_mesh.paint_uniform_color(c)
            bbox3d_list.append(bbox_mesh)
