...
class Model:
    ...
    def __init__(self, pts_feature_num):
        ...
        self.pts_feature_num = pts_feature_num
    ...
    def get_pointcloud_fromfile(self, file_path):
        print(file_path)
        try:
            if file_path.endswith(".bin"):
                pts = np.fromfile(file_path, dtype=np.float32).reshape(-1, self.pts_feature_num)
            ...
            return np.zeros((2, self.pts_feature_num), dtype=np.float32)
        else:
            return pts
    ...
    def read_cams(self, cam_dict):
        ...
        def read_cam(cam):
            cam_ = dict()
            if cam["data_path"] is not None:
                cam_["img"] = np.array(o3d.io.read_image(cam["data_path"]))
            else:
                cam_["img"] = np.zeros([100, 50, 3], dtype=np.uint8)
            # cam_['img'] = np.array(o3d.io.read_image(cam_dict[cam]['data_path']))
            ...
            lidar2cam_rt = cam["ego2cam"]
            intrinsic = cam["intrinsic"]
            lidar2img_rt = intrinsic @ lidar2cam_rt

            cam_["lidar2cam_rt"] = lidar2cam_rt
            cam_["lidar2img_rt"] = lidar2img_rt
            cam_["cam_intrinsic"] = cam["intrinsic"]
            return cam_

        # for cam in cam_dict.keys():
        #     res_dict[cam] = read_cam(cam_dict[cam])

        pool = Pool(thread_pool_num)
        cams = list(cam_dict.keys())
        cams_res = pool.map(read_cam, [cam_dict[cam] for cam in cams])
        for i, cam_res in enumerate(cams_res):
            res_dict[cams[i]] = cam_res
    ...
    def create_point_cloud(self, data):
        ...
        if pts.shape[1] >= 4:
            ...
            xyz = pts[:, [0, 1, 2]]
            tcloud.point["positions"] = Visualizer._make_tcloud_array(xyz, copy=True)
        else:
    ...
    def create_cams(self, name, cam_dict, key="img", update=False, project_pt_size=3):
        ...
        def create_cam(cam):
            if key == "projection":
                if "projection_img" in cam:
                    img_ = cam["projection_img"]
                else:
                    img_ = np.copy(cam["img"])
                    for p in cam[key]:
                        cv2.circle(img_, (int(p[0]), int(p[1])), project_pt_size, (0, 255, 0), -1)
                    cam.update({"projection_img": img_})
                # for ind in range(0, len(v[key]), 20):
                #     p = v[key][ind]
                #     cv2.circle(img_, (int(p[0]), int(p[1])), project_pt_size, (0, 255, 0), -1)
            else:
                img_ = cam[key]
            # img = self._convert_to_numpy(img_)
            if len(img_.shape) == 2 and img_.shape[0] == 1:
                img_ = img_[0]  # "1D" array as 2D: [[1, 2, 3,...]]
            if img_.dtype.name.startswith("int"):
        ...
        # for cam in cams:
        #     cam_res = create_cam(cam_dict[cam])
        #     tcam[cam] = o3d.t.geometry.Image(Visualizer._make_tcloud_array(cam_res))

        pool = Pool(thread_pool_num)
        cams_res = pool.map(create_cam, [cam_dict[cam] for cam in cams])
        for i, cam_res in enumerate(cams_res):
            if key == "projection" and "projection_img" not in cam_dict[cams[i]]:
                del cam_dict[cams[i]]["projection"]
                cam_dict[cams[i]].update({"projection_img": cam_res})
            tcam[cams[i]] = o3d.t.geometry.Image(Visualizer._make_tcloud_array(cam_res))
...
class DataModel(Model):
    ...
    def __init__(self, userdata, pts_feature_num):
        super().__init__(pts_feature_num)
        ...
        self._memory_limit = 2 * 8192 * 1024 * 1024  # memory limit in bytes
    ...
    def load(self, name, fail_if_no_space=False, projection=False, project_pt_size=3):
        ...
        data = self._name2srcdata[name]
        points = self.get_pointcloud_fromfile(data["point"])
        data["points"] = points
        ...
        if "cams" in data and data["cams"] is not None:
            cams = self.read_cams(data["cams"])
            for item in cams:
                data["cams"][item].update(cams[item])
            ...
            if "cams" in data:
                for _, val in data["cams"].items():
                    ...
                    projection_img = self._points_project_to_img(val, data["points"])
                    val["projection"] = projection_img
                    # val['bbox_3d'] = val['img']

                key = "projection" if projection else "img"
                self.create_cams(data["name"], data["cams"], key=key, update=True, project_pt_size=project_pt_size)

        size = self._calc_pointcloud_size(self._data[name], self.tclouds[name], self.tcams[name])
        if size + self._current_memory_usage > self._memory_limit:
            if fail_if_no_space:
                ...
                remove_size = self._calc_pointcloud_size(self._data[remove_name], self.tclouds[remove_name])
    ...
    def _points_project_to_img(self, cam_dict, pcloud):
        """project point cloud to image using param; cam_dict:img, lidar2img_rt, lidar2cam_rt, intrinsic."""
        tmp = np.ones((pcloud.shape[0], 1), dtype=float)
        points = np.concatenate((pcloud[:, :3], tmp), axis=1)
        points = points.transpose()
        points_camera = np.dot(cam_dict["lidar2img_rt"], points).transpose()

        index = points_camera[:, 2] < 1e-2
        points_camera = points_camera[~index, :]
        points_camera[:, 0] = points_camera[:, 0] / points_camera[:, 2]
        points_camera[:, 1] = points_camera[:, 1] / points_camera[:, 2]
        x_filter = (points_camera[:, 0] < 0) | (points_camera[:, 0] > cam_dict["img"].shape[1])
        y_filter = (points_camera[:, 1] < 0) | (points_camera[:, 1] > cam_dict["img"].shape[1])
        index = x_filter | y_filter
        points_camera = points_camera[~index, :]
    ...
    def unload(self, name):
        ...
        if name in self._name2srcdata:
            tcloud = o3d.t.geometry.PointCloud(o3d.core.Device("CPU:0"))
            del self.tclouds[name]
            del self.tcams[name]
            self.tclouds[name] = tcloud
...
class DataModel_Single(Model):
    ...
    def __init__(self, userdata):
        ...
        for d in userdata:
            ...
            if "bounding_boxes" in d:
                self.bounding_box_data.append(Model.BoundingBoxData(name, d["bounding_boxes"]))
...
class DatasetModel(Model):
    ...
    def __init__(self, dataset, split, indices, pts_feature_num):
        super().__init__(pts_feature_num)
        self._dataset = None
        ...
        if "bounding_boxes" in data:
            ...
            if "cams" in data:
                for _, val in data["cams"].items():
                    lidar2img_rt = val["lidar2img_rt"]
                    bbox_data = data["bounding_boxes"]
                    bbox_3d_img = BoundingBox3D.project_to_img(bbox_data, np.copy(val["img"]), lidar2img_rt)
                    val["bbox_3d"] = bbox_3d_img
...
class Visualizer:
    ...
    class LabelLUTEdit:
        ...
        def get_colors(self):
            ...
            return [self._label2color[label] for label in self._label2color.keys()]
        ...
        def set_labels(self, labellut):
            ...
            for key in labellut.labels.keys():
                ...
                print(f"lut:{key},{lbl.name},{lbl.value},{lbl.color}")
                color = lbl.color
                if len(color) == 3:
                    color += [1.0]
                self._label2color[key] = color
                color = gui.Color(lbl.color[0], lbl.color[1], lbl.color[2])
                cell = gui.LUTTreeCell(str(key) + ": " + lbl.name, True, color, None, None)
        ...
        def _on_label_color_changed(self, label, gui_color):
            self._label2color[label] = [gui_color.red, gui_color.green, gui_color.blue, self._label2color[label][3]]
    ...
    class ColormapEdit:
        ...
        def _on_color_changed(self, idx, gui_color):
            self.colormap.points[idx].color = [gui_color.red, gui_color.green, gui_color.blue]
            if self._on_changed is not None:
                self._on_changed()
    ...
    class ProgressDialog:
        ...
        def post_update(self, text=None):
            ...
            if text is None:
                gui.Application.instance.post_to_main_thread(self._window, self.update)
            else:
                ...
                gui.Application.instance.post_to_main_thread(self._window, update_with_text)
    ...
    def __init__(self):
        ...
        self._prev_img_mode = 2
        self.save_dir = "/tmp/UtoVis"
        self.capture_dir = "screen"
        self.data_root = "/home"
        self._keep_capture = False
        # self._look_at_center = [20, 0, 0]
        # self._look_at_eye = [-30, 0, 30]
        # self._fov = 60
        # self._pt_size = 1.5
        # self._look_at_center = [20, 0.4, 2.5]
        # self._look_at_eye = [4.5, 0.4, 2.5]
        self._look_at_center = [50, 0, 0]
        self._look_at_eye = [50, 0, 100]
        self._fov = 60
        self._pt_size = 2
        self._show_boundingbox = True
        self._draw_boundingbox = True
        self._preload_number_step = 50
        self._on_key_freeze = False
        self._panel_em = 35
        self._project_img = True
        self._project_point_size = 3

    def _init_dataset(self, dataset, split, indice, pts_feature_num):
        self._objects = DatasetModel(dataset, split, indice, pts_feature_num)
        self._modality = dict()
        ...
        if hasattr(self._objects._dataset, "infos"):
            ...
            if "cams" in self._objects._dataset.infos[0]:
                ...
                self._cam_names = list(self._objects._dataset.infos[0]["cams"].keys())

    def _init_data(self, data, pts_feature_num):
        self._objects = DataModel(data, pts_feature_num)
        self._modality = dict()
    ...
    def _init_user_interface(self, title, width, height):
        ...
        self._img_panel = gui.CollapsableVert("Cameras", 0, indented_margins)
        self.window.add_child(self._img_panel)
        self._img_panel.visible = False
        ...
        capture = gui.Button("Capture")
        capture.set_on_clicked(self._on_export_screen_image)
        capture.horizontal_padding_em = 0.5
        capture.vertical_padding_em = 0
        h = gui.Horiz(0.25 * em)
        h.add_stretch()
        h.add_child(arcball)
        h.add_child(fly)
        ...
        save.set_on_clicked(self._on_save_file)
        save.horizontal_padding_em = 0.5
        save.vertical_padding_em = 0
        list_selector_grid.add_child(save)
        # forward and backward button
        backward = gui.Button("<")
        backward.set_on_clicked(self._on_prev_file)
        backward.horizontal_padding_em = 0.5
        backward.vertical_padding_em = 0
        list_selector_grid.add_child(backward)
        forward = gui.Button(">")
        forward.set_on_clicked(self._on_next_file)
        forward.horizontal_padding_em = 0.5
        forward.vertical_padding_em = 0
        list_selector_grid.add_child(gui.Label(" "))
        list_selector_grid.add_child(forward)

        # add keyboard control
        self._3d.set_on_key(self._on_key)
        # TODO: solve the problem that you need to click on
        # the point cloud to control with the keyboard
        ...
        # ...  boxes list
        self._boxes = gui.TreeView()
        self._boxes.set_on_selection_changed(self._on_boxes_selection_changed)
        boxes_list = gui.Vert(2)
        boxes_list.add_child(self._boxes)
        view_tab.add_tab("Bboxes", boxes_list)
        ...
        grid.add_child(gui.Label(""))
        grid.add_child(gui.Label("Cam-Setup:"))
        grid.add_child(gui.Label(""))
        grid.add_child(gui.Label("Center"))
        center = gui.VectorEdit()
        center.vector_value = self._look_at_center
        center.set_on_value_changed(self._setup_camera_look_at_center)
        grid.add_child(center)
        grid.add_child(gui.Label("Eye"))
        eye = gui.VectorEdit()
        eye.vector_value = self._look_at_eye
        eye.set_on_value_changed(self._setup_camera_look_at_eye)
        grid.add_child(eye)
        grid.add_child(gui.Label("Fov"))
        fov = gui.NumberEdit(gui.NumberEdit.Type(1))
        fov.double_value = self._fov
        fov.set_on_value_changed(self._setup_camera_fov)
        grid.add_child(fov)
        grid.add_child(gui.Label("Point_size"))
        pt_size = gui.NumberEdit(gui.NumberEdit.Type(1))
        pt_size.double_value = self._pt_size
        pt_size.set_on_value_changed(self._setup_point_size)
        grid.add_child(pt_size)
        # grid.add_child(gui.Label("panel_width(15-55%)"))
        # panel_em = gui.NumberEdit(gui.NumberEdit.Type(1))
        # panel_em.double_value = self._panel_em
        # panel_em.set_on_value_changed(self._setup_panel_em)
        # grid.add_child(panel_em)
        saveimg = gui.Checkbox("AutoCap")
        saveimg.set_on_checked(self._on_checked_keep_capture)
        saveimg.checked = self._keep_capture
        # saveimg.horizontal_padding_em = 0.5
        # saveimg.vertical_padding_em = 0
        grid.add_child(saveimg)
        grid.add_child(gui.Label(""))
        showbbox = gui.Checkbox("ShowBBx")
        showbbox.set_on_checked(self._on_checked_showbbox)
        showbbox.checked = self._show_boundingbox
        # saveimg.horizontal_padding_em = 0.5
        # saveimg.vertical_padding_em = 0
        grid.add_child(showbbox)
        grid.add_child(gui.Label(""))
        drawbbox = gui.Checkbox("DrawBBx")
        drawbbox.set_on_checked(self._on_checked_drawbbox)
        drawbbox.checked = self._draw_boundingbox
        grid.add_child(drawbbox)
        ...
        if "use_camera" in self._modality and self._modality["use_camera"]:
            self._img_panel.visible = True
            # w = gui.CollapsableVert("Cameras", 0, indented_margins)
            vgrid = gui.VGrid(1, 0, indented_margins)
            self._img_panel.add_child(vgrid)
            vgrid.add_child(gui.Label("panel_width"))
            slider = gui.Slider(gui.Slider.INT)
            slider.set_limits(10, 60)
            slider.int_value = self._panel_em
            slider.set_on_value_changed(self._setup_panel_em)
            vgrid.add_child(slider)

            pgrid = gui.VGrid(2, 0, indented_margins)
            self._img_panel.add_child(pgrid)
            # pgrid.add_child(gui.Label("projection"))
            projection = gui.Checkbox("projection")
            projection.set_on_checked(self._on_checked_projection)
            projection.checked = self._project_img
            pgrid.add_child(projection)
            # pgrid.add_child(gui.Label("pc_size"))
            # pslider = gui.Slider(gui.Slider.INT)
            # pslider.set_limits(1, 5)
            # pslider.int_value = self._project_point_size
            # pslider.set_on_value_changed(self._setup_panel_em)
            # pgrid.add_child(pslider)

            cam_grid = gui.VGrid(1, 0, indented_margins)  # change no. of cam_grid columns here
            self._img_panel.add_child(cam_grid)
    ...
    def _setup_camera(self):
        ...
        selected_bounds = [self._objects.calc_bounds_for(n) for n in selected_names]
        ...
        self._3d.setup_camera(self._fov, bounds, bounds.get_center())
        self._3d.look_at(self._look_at_center, self._look_at_eye, [0, 0, 1])

    def _setup_camera_look_at_center(self, center):
        """Set up camera for visualization."""
        print(f"Set camera look_at center: {center}")
        self._look_at_center = center.tolist()
        selected_names = self._get_selected_names()
        selected_bounds = [self._objects.calc_bounds_for(n) for n in selected_names]
        min_val = [1e30, 1e30, 1e30]
        max_val = [-1e30, -1e30, -1e30]
    ...
    def _setup_camera_look_at_eye(self, eye):
        """Set up camera for visualization."""
        print(f"Set camera look_at eye: {eye}")
        self._look_at_eye = eye.tolist()
        selected_names = self._get_selected_names()
        selected_bounds = [self._objects.calc_bounds_for(n) for n in selected_names]
        min_val = [1e30, 1e30, 1e30]
        max_val = [-1e30, -1e30, -1e30]
    ...
    def _setup_camera_fov(self, fov):
        """Set up camera for visualization."""
        print(f"Set camera fov: {fov}")
        self._fov = fov
        selected_names = self._get_selected_names()
        selected_bounds = [self._objects.calc_bounds_for(n) for n in selected_names]
        min_val = [1e30, 1e30, 1e30]
    ...
    def _setup_point_size(self, pt_size):
        """Set up point size for visualization."""
        print(f"Set point size: {pt_size}")
        self._pt_size = pt_size
        self._update_geometry()

    def _setup_panel_em(self, panel_em):
        """Set up point size for visualization."""
        # print(f"Set image panel width: {panel_em}")
        # panel_em = min(max(panel_em, 20), 60)
        self._panel_em = panel_em

    def _setup_projection_point_size(self, pt_size):
        """Set up point size for visualization."""
        # print(f"Set image panel width: {panel_em}")
        # panel_em = min(max(panel_em, 20), 60)
        self._project_point_size = pt_size
        self._check_bw_lims()
    ...
    def _add_tree_name(self, name, is_geometry=True):
        ...
        def on_checked(checked):
            ...
            elif name == Model.bounding_box_prefix.split("/")[0]:
                for bbox_item in self._name_bbox_geo:
                    self._3d.scene.show_geometry(bbox_item, checked)
            self._3d.force_redraw()
    ...
    def _add_boxes_in_selected_names(self):
        boxes = self._get_boxes_in_selected_names()
        parent = self._boxes.get_root_item()
        self._boxes.clear()
        for i in range(len(boxes)):
            box = boxes[i].center.tolist()
            box.extend(boxes[i].size)
            box = [round(_, 2) for _ in box]
            name = f"{boxes[i]}"
            ...
            cell = gui.CheckableTextTreeCell(name, True, on_checked)
            self._boxes.add_item(parent, cell)
    ...
    def _load_geometry(self, name, ui_done_callback):
        ...
        def load_thread():
            names = self._objects.data_names
            current_id = names.index(name)
            # for i in range(current_id - self._preload_number_step, current_id + self._preload_number_step):
            for i in range(current_id - 5, current_id + self._preload_number_step):
                if i >= 0 and i + 1 < len(names):
                    result = self._objects.load(names[i], False, self._project_img, self._project_point_size)
                    print(f"Loading {names[i]}: {result}")
    ...
    def _load_geometries(self, names, ui_done_callback):
        ...
        progress_dlg = Visualizer.ProgressDialog("Loading...", self.window, len(names) + 1)
        progress_dlg.set_text("Loading " + names[0] + "...")
        datamodel = self._objects

        def load_thread():
            def process_job(name):
                text = "Loading " + name + "..."
                progress_dlg.post_update(text)
                result = datamodel.load(name, True, self._project_img, self._project_point_size)
                print(f"Loading {name}: {result}")
                return result

            for i in range(0, len(names)):
                result = process_job(names[i])
                if result:
                    self._name2treenode[names[i]].label.text_color = gui.Color(0.0, 1.0, 0.0, 1.0)
                else:
                    break
    ...
    def _update_geometry(self, check_unloaded=True):
        if check_unloaded:
            for name in self._objects.data_names:
        ...
        for n, tcloud in self._objects.tclouds.items():
            ...
            # update image after ui_initialize
            node = self._name2treenode[n]
            if node is not None and node.checkbox.checked:
                self._update_image(n)
        self._3d.force_redraw()
    ...
    def _update_point_cloud(self, name, tcloud, material):
        ...
        # Update geometry
        material.point_size = self._pt_size
    ...
    def _update_bounding_boxes(self, animation_frame=None):
        ...
        mat_box.base_color = [1.0, 1.0, 1.0, 0.3]
        ...
        if self._consolidate_bounding_boxes:
            ...
            self._3d.scene.remove_geometry(name)
            for bbox_name in self._name_bbox_geo:
                self._3d.scene.remove_geometry(bbox_name)
            self._name_bbox_geo.clear()
            if len(boxes) > 0:
                lines = BoundingBox3D.create_lines(boxes, lut)
                self._3d.scene.add_geometry(name, lines, mat)
                self._3d.scene.show_geometry(name, self._show_boundingbox)
                if self._draw_boundingbox:
                    bboxes = BoundingBox3D.create_bboxes(boxes, lut)
                    for i, bbox in enumerate(bboxes):
                        self._3d.scene.add_geometry(f"{boxes[i]}", bbox, mat_box)  # bbox_{i}
                        self._3d.scene.show_geometry(f"{boxes[i]}", self._show_boundingbox)
                        self._name_bbox_geo.append(f"{boxes[i]}")
        ...
        if self._shader.selected_text == self.LABELS_NAME:
            ...
            if n >= 1:
                self._gradient.points = [
                    rendering.Gradient.Point(float(i) / n, [colors[i][0], colors[i][1], colors[i][2], colors[i][3]])
            ...
            if cmap is not None:
                self._gradient.points = [
                    rendering.Gradient.Point(p.value, [p.color[0], p.color[1], p.color[2], 1.0]) for p in cmap.points
                ]
    ...
    def _on_layout(self, context=None):
        ...
        em = self.window.theme.font_size
        panel_width = 25 * em  # 20 * em
        panel_rect = gui.Rect(frame.get_right() - panel_width, frame.y, panel_width, frame.height - frame.y)
        self._panel.frame = panel_rect
        self._3d.frame = gui.Rect(frame.x, frame.y, panel_rect.get_right() - frame.x, frame.height - frame.y)
        # for img_pannel
        img_panel_height = min(
            frame.height, self._img_panel.calc_preferred_size(context, gui.Widget.Constraints()).height
        )
        img_panel_width = min(
            self._panel_em * em, self._img_panel.calc_preferred_size(context, gui.Widget.Constraints()).width
        )
        self._img_panel.frame = gui.Rect(0, 0, img_panel_width, img_panel_height)
        # _3d_width = 50 * em
        # self._3d.frame = gui.Rect(frame.x, frame.y,
        #                           _3d_width, frame.height - frame.y)
        # panel_rect = gui.Rect(_3d_width, frame.y,
        #                       frame.get_right() - _3d_width, frame.height - frame.y)
        # self._panel.frame = panel_rect

    def _on_export_screen_image(self):
        def on_image(image):
            current_idx = self._lower_val.int_value
            current_name = self._objects.data_names[current_idx]
            if not os.path.exists(self.capture_dir):
                os.makedirs(self.capture_dir)
            o3d.io.write_image(f"{self.capture_dir}/{current_name}.jpg", image, 100)

        self._3d.scene.scene.render_to_image(on_image)

    def _on_checked_keep_capture(self, checked):
        self._keep_capture = checked

    def _on_checked_showbbox(self, checked):
        self._show_boundingbox = checked
        if "Bounding Boxes" in self._name2treenode:
            self._name2treenode["Bounding Boxes"].checkbox.checked = checked
        self._update_bounding_boxes()

    def _on_checked_drawbbox(self, checked):
        self._draw_boundingbox = checked
        self._update_bounding_boxes()

    def _on_checked_projection(self, checked):
        self._project_img = checked
        if self._project_img:
            self._on_img_mode_changed("projection", 2)
        else:
            self._on_img_mode_changed("raw", 0)
        # self._check_bw_lims()
    ...
    def _on_dataset_selection_changed(self, item):
        ...
        if not self._objects.is_loaded(name):
            self._on_key_freeze = True
            self._load_geometry(name, ui_callback)

    def _on_boxes_selection_changed(self, item):
        # TODO: donot forget this func
        self._update_bounding_boxes()

    def _on_save_file(self):
        names = self._get_selected_names()
        for name in names:
            pts_src = f"{self.data_root}/Lidar"
            pts_dst = f"{self.save_dir}/Lidar"
            if not os.path.exists(pts_dst):
                os.makedirs(pts_dst)
            shutil.copy(f"{pts_src}/{name}", f"{pts_dst}/{name}")
            txt_src = f"{self.data_root}/annotation"
            txt_dst = f"{self.save_dir}/annotation"
            if not os.path.exists(txt_dst):
                os.makedirs(txt_dst)
            shutil.copy(f"{txt_src}/{name[:-4]}.txt", f"{txt_dst}/{name[:-4]}.txt")
    ...
    def _on_display_tab_changed(self, index):
        if index == 1:
            ...
            self._add_boxes_in_selected_names()
        else:
    ...
    def _on_animation_slider_changed(self, new_value):
        ...
        if "use_camera" in self._modality and self._modality["use_camera"]:
            for cam in self._cam_names:
                if cam in self._objects.tcams[self._animation_frames[idx]]:
                    self._img[cam].update_image(self._objects.tcams[self._animation_frames[idx]][cam])
        ...
        self._slider_current.frame = gui.Rect(r.x, r.y, self._slider.frame.get_right(), r.height)

    def _update_image(self, name):
        if "use_camera" in self._modality and self._modality["use_camera"]:
            for cam in self._cam_names:
                if cam in self._objects.tcams[name]:
                    self._img[cam].update_image(self._objects.tcams[name][cam])
    ...
    def _on_img_mode_changed(self, name, idx):
        if idx == self._prev_img_mode:
            return
        if not "use_camera" in self._modality or not self._modality["use_camera"]:
            return
        ...
        if idx == 0 or name == "raw":
            ...
            key = "bbox_3d"
        elif idx == 2 or name == "projection":
            key = "projection"

        # progress_dlg = Visualizer.ProgressDialog("Processing images...", self.window, 2)
        # progress_dlg.set_text("Processing images " + name + "...")

        # def ui_done_callback():
        #     self._check_bw_lims()

        n = self._objects.data_names[self._upper_val.int_value]
        self._objects.create_cams(
            n, self._objects._data[n]["cams"], key=key, update=False, project_pt_size=self._project_point_size
        )
        self._update_image(n)

        def process_thread():
            for n in self._objects.data_names:
                print(f"Processing image {n}: {key}")
                # text = "Processing image " + n + "..."
                # progress_dlg.post_update(text)
                if self._objects.is_loaded(n):
                    self._objects.create_cams(
                        ...
                        key=key,
                        update=False,
                        project_pt_size=self._project_point_size,
                    )
    ...
    def _on_bgcolor_changed(self, new_color):
        bg_color = [new_color.red, new_color.green, new_color.blue, new_color.alpha]
        self._3d.scene.set_background(bg_color)
        self._3d.force_redraw()

    def _on_key(self, e):
        if not self._on_key_freeze:
            if e.key == gui.KeyName.RIGHT or e.key == gui.KeyName.DOWN:
                if e.type == gui.KeyEvent.UP:  # check UP so we default to DOWN
                    self._on_next_file()
                return gui.Widget.EventCallbackResult.HANDLED
            if e.key == gui.KeyName.LEFT or e.key == gui.KeyName.UP:
            ...
            if e.key == gui.KeyName.C:
                if e.type == gui.KeyEvent.UP:
                    self._on_export_screen_image()
                return gui.Widget.EventCallbackResult.HANDLED
        return gui.Widget.EventCallbackResult.IGNORED

    def _on_prev_file(self):
        if self._lower_val.int_value < 1:
            pass
        else:
            self._lower_val.int_value -= 1
            self._upper_val.int_value = self._lower_val.int_value
        self._on_lower_val(self._prev_lower_val)
        self._update_bounding_boxes()
        if self._keep_capture:
            self._on_export_screen_image()

    def _on_next_file(self):
        if self._upper_val.int_value >= self._upper_val.maximum_value:
            pass
        else:
            self._upper_val.int_value += 1
            self._lower_val.int_value = self._upper_val.int_value
        self._on_upper_val(self._prev_upper_val)
        self._update_bounding_boxes()
        if self._keep_capture:
            self._on_export_screen_image()
    ...
    def _get_boxes_in_selected_names(self):
        boxes = []
        # for name in self._get_selected_names():
        #     idx = self._objects.data_names.index(name)
        #     idx -= (len(self._objects.data_names) -
        #             len(self._objects.bounding_box_data))
        #     boxes.extend(self._objects.bounding_box_data[idx].boxes)
        for bbox_item in self._objects.bounding_box_data:
            if bbox_item.name in self._get_selected_names():
                boxes.extend(bbox_item.boxes)
        return boxes
    ...
    def visualize_dataset(self, dataset, split, indices=None, pts_feature_num=6, width=1280, height=768):
        """Visualize a dataset.
        ...
        for val in dataset.label_to_names.values():
            lut.add_label(val, val, color=[1, 0, 0])
        self.set_lut("labels", lut)

        self._consolidate_bounding_boxes = True
        self._init_dataset(dataset, split, indices, pts_feature_num)
        self._visualize("Open3D - " + dataset.name, width, height)

    def visualize(self, data, lut=None, pts_feature_num=6, bounding_boxes=None, width=1280, height=768):
        ...
        """
        self._init_data(data, pts_feature_num)
        ...
        if bounding_boxes is not None:
            ...
            if group_size < 2:
                box_data = [Model.BoundingBoxData(prefix + str(bbox), [bbox]) for bbox in bounding_boxes]
            else:
                box_data = []
                current_group = []
